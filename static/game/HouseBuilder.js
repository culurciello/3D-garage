...
// https://playground.babylonjs.com/#4GBWI5#272 
...
export default class HouseBuilder {
    ...
    create() {
        ...
        for(b = 0; b < baseData.length/2; b++) {
            corners.push(this.corner(baseData[2*b], baseData[2*b + 1]));
        }
        
        var door = this.door(1, 1.8);
        var doorSpace = this.doorSpace(door, 1);
        
        var window0 = this.window(1.2, 2.4);
        var window1 = this.window(2, 2.4);
        
        var windowSpace02 = this.windowSpace(window0, 0.814, 0.4);
        var windowSpace1 =  this.windowSpace(window0, 0.4, 0.4);
        var windowSpace78 = this.windowSpace(window1, 1.5, 0.4);
        
        var walls = [];
        for(c=0; c<corners.length; c++) {
            walls.push(this.wall(corners[c]));
        }
        ...
        var house = this.buildFromPlan(walls, ply, height, 
            {interiorUV: new BABYLON.Vector4(0.167, 0, 1, 1), 
             exteriorUV: new BABYLON.Vector4(0, 0, 0.16, 1)}, 
             this.scene);
        
        var mat = new BABYLON.StandardMaterial("bobho", this.scene);
        mat.diffuseTexture = new BABYLON.Texture("./static/assets/textures/88fOIk3.jpg", this.scene);
        
        house.material = mat;
        var m = 4; // multiplier scaling house
        house.scaling = new BABYLON.Vector3(m, m, m);
    ...
    wall(corner, doorSpaces, windowSpaces) {
        var d = {};
        d.corner = corner;
        d.doorSpaces = doorSpaces || [];
        d.windowSpaces = windowSpaces || []; 
        return d;
    }


    buildFromPlan(walls, ply, height, options, scene) {
        ...
        for(var w = 0; w < nbWalls - 1; w++) {
            ...
            polygonTriangulation = new BABYLON.PolygonMeshBuilder("boh", polygonCorners, scene);   
        ...
        var customMesh = new BABYLON.Mesh("room", scene);
        ...
        // customMesh.destructible = false; // can be destroyed 
        // customMesh.destructive = false; // can destroy
        // customMesh.visible = true;
        // customMesh.solid = true;
        // customMesh.movable = false; // movable
